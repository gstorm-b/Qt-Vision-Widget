...
bool CommandModel::moveUp(const QModelIndex& index) {
  ...
  if (!p) return false;
  const int r = n->row();
  // if current node is child of root => limit move up to row 1,
  // that mean current position valid is below 2
  // else limit move up to 0
  const bool parentIsRoot = (p->parent() == nullptr);
  const int offMoveLimit = parentIsRoot ? 2 : 1;
  if (r < offMoveLimit) return false;
