...
public:
  explicit CommandRowWidget(QWidget* parent = nullptr)
    ...
    lblOrder->setMinimumWidth(25);
    QFont order_font = lblOrder->font();
    order_font.setBold(true);
    order_font.setItalic(true);
    lblOrder->setFont(order_font);

    spacer = new QSpacerItem(10, 10, QSizePolicy::Fixed, QSizePolicy::Fixed);

    /// command title label text
    lblTypeName = new QLabel("", this);
    lblTypeName->setMinimumWidth(42);
    order_font.setItalic(false);
    lblTypeName->setFont(order_font);
    ...
    h->addWidget(lblOrder, 0, Qt::AlignLeft);
    h->addWidget(lblTypeName, 0, Qt::AlignLeft);
  ...
  void refresh() {
    ...
    QString title = c ? ( QString("[%1]: %2")
                                             .arg(c->commandName())
                                             .arg(c->info()) )
                      : QString("Command error");
    lblTypeName->setText(c->typeName());
    lblTitle->setText(title);

    // get global index
    int order = m_model->globalOrder(node, /*includeStart=*/false);
    if (isStart) {
      // start node order string
      lblOrder->setText("0");
